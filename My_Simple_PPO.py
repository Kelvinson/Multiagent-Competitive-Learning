...
for ep in range(EP_MAX):
    ...
    for t in range(EP_LEN):    # in one episode
        ...
        bs, ba, br = [np.zeros(3,1) for i in range(len1)]
        if (t+1) % BATCH == 0 or t == EP_LEN-1:
            ...
            for i in range(len1):
                ...
                bs[i], ba[i], br[i] = np.vstack(buffer_s[i]), np.vstack(buffer_a[i]), np.array(discounted_r[i])[:, np.newaxis]
                # buffer_s, buffer_a, buffer_r = [[] for i in range(len1)], [[] for i in range(len1)], [[] for i in range(len1)]
                buffer_s[i], buffer_a[i], buffer_r[i] = [], [], []
                # for i in range(len1):
                ppo[i].update(bs[i], ba[i], br[i])
    if ep == 0:
        for i in range(len1):
...
plt.plot(np.arange(len(all_ep_r[0])), all_ep_r)
